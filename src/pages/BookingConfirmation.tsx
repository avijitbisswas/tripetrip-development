        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
          <article className="overflow-hidden rounded-[34px] border border-emerald-100 bg-white shadow-[0_28px_100px_rgba(15,23,42,0.12)]">
            <div className="relative min-h-[260px] p-6 sm:p-8">
              <img src={featuredAdventure.image} alt={featuredAdventure.title} className="absolute inset-0 h-full w-full object-cover" />
              ...
              <div className="relative max-w-xl text-white">
                ...
                <h2 className="mt-5 text-3xl font-black tracking-tight sm:text-4xl">{featuredAdventure.title}</h2>
                <p className="mt-3 flex items-center gap-2 text-sm font-bold text-white/85"><MapPin className="h-4 w-4" /> {featuredAdventure.location}</p>
                <div className="mt-6 grid max-w-md grid-cols-3 gap-3 text-center">
                  <PassStat label="Date" value="24 May" />
                  <PassStat label="Slot" value="10 AM" />
                  <PassStat label="Guests" value="2" />
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-5 p-6 sm:p-8 md:grid-cols-[1fr_220px]">
              <div>
                ...
                <p className="mt-2 text-3xl font-black tracking-tight text-[#059669]">THR12345678</p>
                <div className="mt-5 grid gap-3 sm:grid-cols-2">
                  <InfoLine icon={ShieldCheck} label="Operator" value={featuredAdventure.operator.name} />
                  <InfoLine icon={Clock} label="Duration" value={featuredAdventure.duration} />
                  <InfoLine icon={MapPin} label="Meeting Point" value="Bir Landing Site" />
                  <InfoLine icon={TicketCheck} label="Total Paid" value={formatRupees(featuredAdventure.directPrice * 2)} />
                </div>
              </div>
          ...
          <aside className="rounded-[34px] border border-slate-200 bg-white p-5 shadow-[0_24px_80px_rgba(15,23,42,0.1)]">
            ...
            <p className="mt-1 text-sm font-semibold text-slate-500">We have sent the ticket and operator chat link to your email.</p>
            ...
            <div className="mt-6 rounded-[24px] bg-emerald-50 p-4">
              ...
              <div className="mt-3 space-y-3 text-xs font-bold leading-5 text-slate-600">
                <p><ShieldCheck className="mr-2 inline h-4 w-4 text-[#059669]" /> Confirmation sent to rohit.s@email.com.</p>

...
describe('AccountingWorkspace', () => {
  ...
  it('shows accommodation finance guidance for approvals and billing features', () => {
    ...
    expect(screen.getByText('Refund controls')).toBeInTheDocument();
    expect(screen.getAllByText('Night audit').length).toBeGreaterThan(0);
    expect(screen.getAllByText('Open').length).toBeGreaterThan(0);
  });

  it('keeps settlement capture available when refund and night-audit controls are locked', async () => {
    vi.mocked(listVendorAccountingRecords).mockResolvedValueOnce([
      {
        id: 'payment-1',
        organization_id: 'org-1',
    ...
    expect(await screen.findByText('Night audit is locked on the current accommodation plan. Core invoicing and payment capture remain available.')).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Refund Payment' })).not.toBeInTheDocument();
    expect(screen.getByText('Refund controls are locked on the current accommodation plan.')).toBeInTheDocument();
